...
The core entity is `tea` — one record per product in a user's collection. We need `user` for ownership scoping. Freshness status is computed at read time rather than stored, which avoids stale cached values and keeps the write path simple. The `freshness_window_days` field carries type-specific defaults so that users don't need to know shelf life numbers themselves.
